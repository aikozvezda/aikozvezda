    ...
    with open(filename) as f:
        for line in f.readlines():
            ...
            if items[1] == 'M':
                ...
                wdbc.target.append(1)        # TODO #1) Add the true label (0 for M / 1 for others)
            values = []
            for value in items[2:]:
                values.append(float(value))
            wdbc.data.append(values)   
            #while True:
            #    float.items[2:]           # TODO #1) Add 30 attributes (as floating-point numbers)
...
if __name__ == '__main__':
    ...
    # Train a model
    model = svm.SVC(kernel='poly',degree=2)                           # TODO #2) Find a better classifier (SVC accuracy: 0.902)
